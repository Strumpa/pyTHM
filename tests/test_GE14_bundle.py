...
@pytest.fixture(scope='session')
def solver_data_longer_channel():
    yaml_path = os.path.join(GE14_DATA , "GEOM_BUNDLE_MPHYS_TEST.yaml")
    ...
    solver_data = analyser.run_THM_analysis(nz=128, include_water_rods=True) 
    return solver_data
...
def test_GE14_dfmp_3p6MW_s_GEramp(solver_data_longer_channel, fast_iapws):

    # --- Test case definition ---
    mass_flow = (1827 / 240) * 0.88  # kg/s, 1827 kg/s for full core, 240 assemblies, 88% flow to active flow, no Water Rods
    power_kw = 3.6e3 # assume channel gets 3.6MWth
    power_profile = 'sine'
    pdrop = 1
    frfaccorel_choice = 'Churchill'
    ...
    THsolve = pyTHM_solver(
        case_name=case_name,
        channel_type="square",
        geometric_data=solver_data_longer_channel,
        tInlet=543.15,
        pOutlet=7.20E+06,
        qFlow=mass_flow,
        Powtot=power_w,
    ...
    Teff, Twater, rho, voidFrac, P, U, H = THsolve.get_TH_parameters()
    deltaP = P[0] - P[-1]
    assert voidFrac[-1] == pytest.approx(0.82368, abs=1e-5)
    assert deltaP == pytest.approx(37018, abs=1)